...
Sample2MidiAudioProcessorEditor::Sample2MidiAudioProcessorEditor (Sample2MidiAudioProcessor& p)
    : AudioProcessorEditor (&p), audioProcessor (p), 
      waveformDisplay(p.getFormatManager(), thumbnailCache)
{
    // Styling
    getLookAndFeel().setColour(juce::ResizableWindow::backgroundColourId, juce::Colour(0xff121212));
    getLookAndFeel().setColour(juce::TextButton::buttonColourId, juce::Colour(0xff1f1f1f));
    ...
    exportButton.onMouseDown = [this](const juce::MouseEvent&) {
...
void Sample2MidiAudioProcessorEditor::paint (juce::Graphics& g) {
    ...
    g.setFont(20.0f);
    g.drawText("Sample2MIDI", getLocalBounds().removeFromTop(40), juce::Justification::centred);
}
...
void Sample2MidiAudioProcessorEditor::resized() {
    ...
    scaleDropdown.setBounds(controls.removeFromTop(40).removeFromLeft(150).reduced(2));
    rangeDropdown.setBounds(controls.removeFromLeft(150).reduced(2));
    
    exportButton.setBounds(bottomStrip.reduced(5));
}

bool Sample2MidiAudioProcessorEditor::isInterestedInFileDrag (const juce::StringArray& files) {
    return AudioFileLoader::isSupportedFile(files[0]);
}

void Sample2MidiAudioProcessorEditor::filesDropped (const juce::StringArray& files, int x, int y) {
