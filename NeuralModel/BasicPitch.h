...
public:
  ...
  /**
   * Resets all states of model, clear the posteriorgrams vector computed by the
   * CNN and the note event vector.
   */
  void reset();

  /**
   * Set parameters for next transcription or midi update.
   * @param inNoteSensitivity Note sensitivity threshold (0.05, 0.95). Higher
   * gives more notes.
   * @param inSplitSensitivity Split sensitivity threshold (0.05, 0.95). Higher
   * will split note more, lower will merge close notes with same pitch
   * @param inMinNoteDurationMs Minimum note duration to keep in ms.
   */
  void setParameters(float inNoteSensitivity, float inSplitSensitivity,
                     float inMinNoteDurationMs);

  /**
   * Transcribe the input audio. The note event vector can be obtained after
   * this with getNoteEvents
